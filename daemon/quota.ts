...
export const captureQuota = async (cfg: Config, log: Logger): Promise<QuotaReport> => {
  ...
  const created = await runTmux(["new-session", "-d", "-s", session, "-x", "200", "-y", "50", "-c", cwd, ...paneEnv, "-P", "-F", "#{pane_id}"]);
  ...
  try {
    await runTmux(["send-keys", "-t", pane, `DISABLE_AUTOUPDATER=1 ${claudeBin}`, "Enter"]);
